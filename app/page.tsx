      ...
      <section className="relative overflow-hidden">
        ...
        <div className="mx-auto max-w-6xl px-6 pt-20 pb-16">
          ...
          <div className="inline-flex items-center gap-2 rounded-full border border-indigo-200 bg-gradient-to-r from-indigo-50 to-purple-50 px-4 py-1.5 text-xs text-slate-700 backdrop-blur shadow-sm animate-fade-in">
            ...
            <span className="font-medium">{i18n.badgeNew}</span>
            <span className="text-slate-400">{i18n.badgeSeparator}</span>
            {i18n.badgeText} <span className="font-mono font-semibold text-indigo-600">{i18n.badgeExample}</span>
          </div>
          ...
          <div className="mt-10 flex flex-wrap gap-4 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
            ...
            >
              <span className="relative z-10">{i18n.ctaDemo}</span>
          ...
          <div className="mt-16 grid gap-4 sm:grid-cols-3 animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
            {[
              { icon: i18n.statsSetupIcon, value: i18n.statsSetupValue, label: i18n.statsSetupLabel },
              { icon: i18n.statsUrlIcon, value: i18n.statsUrlValue, label: i18n.statsUrlLabel },
              { icon: i18n.statsUiIcon, value: i18n.statsUiValue, label: i18n.statsUiLabel },
            ].map((stat, i) => (
              <div
                key={stat.label}
              ...
              >
                ...
                <div className="text-lg font-bold text-slate-900 group-hover:text-indigo-600 transition-colors">{stat.value}</div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-xl sm:p-12 hover:shadow-2xl transition-shadow duration-500">
          ...
          <div id="try" className="grid gap-10 lg:grid-cols-2 lg:items-start">
            <div className="space-y-6">
              <div>
                <h2 className="text-2xl font-bold tracking-tight text-slate-900 bg-gradient-to-r from-slate-900 to-indigo-900 bg-clip-text text-transparent">
                  {i18n.trySectionTitle}
                </h2>
                <p className="mt-3 text-base text-slate-600 leading-relaxed">
                  {i18n.trySectionSubtitle}{" "}
                  <span className="font-semibold text-indigo-600">{i18n.trySectionSubtitleHighlight}</span> {i18n.trySectionSubtitleEnd}
                </p>
              </div>
              ...
              <div className="flex gap-3">
                <input
                  ...
                  placeholder={i18n.trySectionPlaceholder}
                ...
                >
                  {i18n.trySectionButton}
                </button>
              </div>

              {/* Demo profiles */}
              <div className="flex flex-wrap gap-2 text-sm">
                {["rvalluri", "johndoe", "janedoe"].map((u) => (
                  <Link
                    key={u}
                    href={`/u/${u}`}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {[
                { icon: i18n.featureHostingIcon, title: i18n.featureHostingTitle, description: i18n.featureHostingDescription, color: "from-blue-500 to-cyan-500" },
                { icon: i18n.featureTrackerIcon, title: i18n.featureTrackerTitle, description: i18n.featureTrackerDescription, color: "from-purple-500 to-pink-500" },
                { icon: i18n.featureResumeIcon, title: i18n.featureResumeTitle, description: i18n.featureResumeDescription, color: "from-orange-500 to-red-500" },
                { icon: i18n.featureOneLinkIcon, title: i18n.featureOneLinkTitle, description: i18n.featureOneLinkDescription, color: "from-green-500 to-emerald-500" },
              ].map((f) => (
                <div
                  key={f.title}
          ...
          <div className="mt-12 flex flex-col justify-between gap-6 rounded-3xl border-2 border-indigo-100 bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 p-8 sm:flex-row sm:items-center shadow-lg hover:shadow-xl transition-all duration-300">
            ...
            >
              {i18n.ctaBottomButton}
              <span className="text-lg">{i18n.ctaBottomArrow}</span>
            </Link>
          </div>
        </div>
      </section>
